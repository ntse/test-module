locals {
  sns_delivery_alarm_topics = var.enable_sns_volume_alarms ? { for k, v in local.sns_topics_allowlisted : k => v if try(var.sns_topic_alarm_config[k].enabled, false) } : {}
}

resource "aws_cloudwatch_metric_alarm" "number_of_messages_published" {
  for_each = local.sns_topics_allowlisted
