...
locals {
  ...
  sns_topics_allowlisted = {
    # There's not much point alarming about the alarm being broken using the broken alarming mechanism so
    # the alarming alarm is filtered out.
    for arn in local.sns_topics :
    arn => {
      topic_name = element(split(":", arn), length(split(":", arn)) - 1)
    }
    if arn != local.sns_topic_arn
  }
